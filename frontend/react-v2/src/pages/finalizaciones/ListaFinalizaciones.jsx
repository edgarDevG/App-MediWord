...
const KPI_THEMES = {
  violet: { title: '#7c3aed', bar: '#a78bfa', value: '#6d28d9', ghost: '#8b5cf6' },
  slate:  { title: '#64748b', bar: '#cbd5e1', value: '#334155', ghost: '#0f172a' },
};

function KpiCard({ label, valor, sub, variant = 'violet', icon }) {
  const [hovered, setHovered] = useState(false);
  const t = KPI_THEMES[variant] ?? KPI_THEMES.violet;
  return (
    <div
      style={{
      ...
      {!kpiLoad && kpis && (
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: '1.5rem', marginBottom: '2rem' }}>
          <KpiCard
            ...
            valor={kpis.totales?.finalizados ?? 0}
            sub="Contratos terminados"
            variant="violet"
            icon="event_busy"
          />
          <KpiCard
